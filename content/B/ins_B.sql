...
IF OBJECT_ID ('dbo.ins_B') IS NOT NULL 
     DROP PROCEDURE dbo.ins_B
GO

CREATE PROCEDURE dbo.ins_B(@branch_id NVARCHAR(50), @id int, @select nvarchar(255))
AS
BEGIN
    ...
    BEGIN TRANSACTION
	   ...
	   IF EXISTS(SELECT id FROM dbo.B WHERE id = @id AND branch_id = @branch_id)
		  BEGIN
			 set @msg = 'ERROR: B (id: ' + CAST(@id AS NVARCHAR(MAX)) + ') already exists';
			 THROW 50000, @msg, 1
		  END
	   ELSE
		  BEGIN
