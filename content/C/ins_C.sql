...
IF OBJECT_ID ('dbo.ins_C') IS NOT NULL 
     DROP PROCEDURE dbo.ins_C
GO

CREATE PROCEDURE dbo.ins_C(@branch_id NVARCHAR(50), @_123 nvarchar(255), @_4 nvarchar(255))
AS
BEGIN
    ...
    BEGIN TRANSACTION
	   ...
	   IF EXISTS(SELECT _123 FROM dbo.C WHERE _123 = @_123 AND branch_id = @branch_id)
		  BEGIN
			 set @msg = 'ERROR: C (_123: ' + CAST(@_123 AS NVARCHAR(MAX)) + ') already exists';
			 THROW 50000, @msg, 1
		  END
	   ELSE
		  BEGIN
