...
IF OBJECT_ID ('dbo.get_history_<xsl:value-of select="@table_name" />') IS NOT NULL 
     DROP PROCEDURE dbo.get_history_<xsl:value-of select="@table_name" />
GO
CREATE PROCEDURE dbo.get_history_<xsl:value-of select="@table_name" />
    ...
    BEGIN TRANSACTION
	   ...
	   BEGIN
		  ...
		  IF ((@dir &lt;&gt; 'asc') AND (@dir &lt;&gt; 'desc')) BEGIN
			 ...
			 THROW 50000, @msg, 1
		  END
		  EXEC('declare @temp nvarchar(1) = (SELECT top 1 left(cast(' + @col_name_s + ' as nvarchar(max)), 1) FROM dbo.[hist_<xsl:value-of select="@table_name" />])')
		  set @i_s = @i_s + 1
	   END
	   ...
	   BEGIN
		  ...
		  EXEC('declare @temp nvarchar(1) = (SELECT top 1 left(cast(' + @col_name_f + ' as nvarchar(max)), 1) FROM dbo.[hist_<xsl:value-of select="@table_name" />] WHERE branch_name like ''' + @regex + ''')')
		  set @i_f = @i_f + 1
	   END

	   -- Build query
	   DECLARE @select nvarchar(max) = 'SELECT <xsl:for-each select="columns/column" >[<xsl:value-of select="@column_name" />]<xsl:if test="position() != last()">, </xsl:if></xsl:for-each> FROM dbo.[hist_<xsl:value-of select="@table_name" />]';
