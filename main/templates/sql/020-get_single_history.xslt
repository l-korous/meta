...
IF OBJECT_ID ('dbo.get_single_history_<xsl:value-of select="@table_name" />') IS NOT NULL 
     DROP PROCEDURE dbo.get_single_history_<xsl:value-of select="@table_name" />
GO
CREATE PROCEDURE dbo.get_single_history_<xsl:value-of select="@table_name" />
(
...
BEGIN
    ...
    BEGIN TRANSACTION
	   ...
	   END
	   -- Record exists
	   IF NOT EXISTS (select * from dbo.[hist_<xsl:value-of select="@table_name" />] where
       <xsl:for-each select="columns/column[@is_part_of_primary_key=1]" >
