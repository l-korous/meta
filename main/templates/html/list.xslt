        ...
        <script>
            function getQueryString() {
                ...
                <xsl:for-each select="columns/column[@is_primary_key=1]">
                    if(urlParams.get('<xsl:value-of select="column_name" />') == '')
                        return defaultQueryString;
            ...
            function loadCall() {
				...
				loadListData('http://<xsl:value-of select="//configuration[@key='NodeJsHostname']/@value" />:<xsl:value-of select="//configuration[@key='NodeJsPort']/@value" />/api/master/<xsl:value-of select="@table_name" />' + query, function(item) {
					...
					$(newElement).find('.entryDeleter')[0].onclick = function() {
						...
						<xsl:for-each select="columns/column[@is_primary_key=1]">deleteHref = deleteHref + '/' +  item.<xsl:value-of select="@column_name" />;</xsl:for-each>
						delete_item(deleteHref, function() { location.reload(); });
					}
					<xsl:for-each select="columns/column[@is_primary_key=1]">
                    ...
                    <!-- take the other table and fire a GET for it -->
                    ...
                    address_to_load = (`http://<xsl:value-of select="//configuration[@key='NodeJsHostname']/@value" />:<xsl:value-of select="//configuration[@key='NodeJsPort']/@value" />/api/master/<xsl:value-of select="$ref_table_name" />?<xsl:for-each select="reference_details/reference_detail">
                        ...
                        `<xsl:if test="position() != last()"><xsl:text disable-output-escaping="yes">&amp;</xsl:text></xsl:if></xsl:for-each>`).replace(/(\r\n|\n|\r)/gm," ").replace(/\s+/g," ").replace(' ', '').replace(/\s/g, "");
                    loadListData(address_to_load, function(childItem) {
                        ...
                        linkHref = linkHref + '<xsl:value-of select="@column_name" />=' + childItem.<xsl:value-of select="@column_name" /><xsl:if test="position() != last()"><xsl:text disable-output-escaping="yes">+ '&amp;'</xsl:text></xsl:if>;
                        </xsl:for-each>
                        $(newChildElement).find('.entryLinkA')[0].href = linkHref.replace(/(\r\n|\n|\r)/gm," ").replace(/\s+/g," ").replace(' ', '');
