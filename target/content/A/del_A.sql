...
IF OBJECT_ID ('dbo.del_A') IS NOT NULL 
     DROP PROCEDURE dbo.del_A
GO

CREATE PROCEDURE dbo.del_A
...
BEGIN
    ...
    BEGIN TRANSACTION
	   ...
	   END
	   -- Record exists
	   IF NOT EXISTS (select * from dbo.A where id = @id AND branch_id = @branch_id) BEGIN
...
IF OBJECT_ID('TRG_del_A') IS NOT NULL
DROP TRIGGER TRG_del_A
GO

CREATE TRIGGER TRG_del_A
ON dbo.A
AFTER DELETE AS
BEGIN
    ...
    LEFT JOIN dbo.hist_A _h_branch
