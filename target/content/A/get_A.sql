...
IF OBJECT_ID ('dbo.get_A') IS NOT NULL 
     DROP PROCEDURE dbo.get_A
GO

CREATE PROCEDURE dbo.get_A
(@branch_id NVARCHAR(50), @jsonParams nvarchar(max))
AS
BEGIN
    SET XACT_ABORT, NOCOUNT ON
    DECLARE @msg nvarchar(255), @debug nvarchar(max)
    BEGIN TRY
    BEGIN TRANSACTION
	   -- SANITY CHECKS
	   -- Branch exists
	   ...
	   BEGIN
		  ...
		  IF ((@dir <> 'asc') AND (@dir <> 'desc')) BEGIN
			 ...
			 THROW 50000, @msg, 1
		  END
		  EXEC('declare @temp nvarchar(1) = (SELECT top 1 left(cast(' + @col_name_s + ' as nvarchar(max)), 1) FROM dbo.A)')
		  set @i_s = @i_s + 1
	   END
	   ...
	   BEGIN
		  ...
		  EXEC('declare @temp nvarchar(1) = (SELECT top 1 left(cast(' + @col_name_f + ' as nvarchar(max)), 1) FROM dbo.A WHERE branch_id like ''' + @regex + ''')')
		  set @i_f = @i_f + 1
	   END

	   -- Build query
	   DECLARE @select nvarchar(max) = 'SELECT * FROM dbo.A where ';
	   set @i_f = 1
	   WHILE @i_f <= (select count(*) from @FilterBy)
	   BEGIN
		  set @col_name_f = (select col from @FilterBy where _id = @i_f)
