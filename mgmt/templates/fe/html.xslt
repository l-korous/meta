...
<xsl:result-document method="xml" href="index.html">
<html>
    <head>
        <meta charset="utf-8" />
        <meta name="viewport" content="initial-scale=1.0, maximum-scale=1.0, user-scalable=0, width=device-width" />
        <title id="title">META | Index</title>
...
<html>
    <head>
        <meta charset="utf-8" />
        <meta name="viewport" content="initial-scale=1.0, maximum-scale=1.0, user-scalable=0, width=device-width" />
        <xsl:element name="title">META | <xsl:value-of select="@table_name" /></xsl:element>
        <link rel="stylesheet" type="text/css" href="style.css" />
        <script src="jquery-3.3.1.slim.min.js">;</script>
        <script src="functions.js">;</script>
        <xsl:element name="script"><xsl:attribute name="src">functions_<xsl:value-of select="@table_name" />.js</xsl:attribute>;</xsl:element>
    </head>
    <body onload="loadCall()" onkeypress="handleKeyPress(event)" id="body">
        <nav>
            <xsl:for-each select="//table" >
                <div class="navLinkBundle">
                    <xsl:element name="button">
            ...
            <div class="entry bubblar" id="entry-dummy">
                ...
                <div class="read">
                    ...
                    <xsl:for-each select="//table//columns/column[@referenced_table_name=$table_name]" >
                        ...
                        <xsl:element name="div">
                            <xsl:attribute name="class">entryLinks entryLinks<xsl:value-of select="$referencing_column_name" /></xsl:attribute>
                            ...
                            <xsl:element name="button">
                                    ...
                                    <xsl:attribute name="id">addEntryLink<xsl:value-of select="$referencing_column_name" /></xsl:attribute>
                                    Add <xsl:value-of select="$referencing_table_name" />
                            </xsl:element>
                            
                            <xsl:element name="div">
                                <xsl:attribute name="class">entryLinkDiv entryLinkDiv<xsl:value-of select="$referencing_column_name" /></xsl:attribute>
                                <xsl:attribute name="id">entryLinkDiv<xsl:value-of select="$referencing_column_name" />-dummy</xsl:attribute>
                                <button class="entryLinkDeleter">Delete</button>
                                <xsl:element name="a">
                                    <xsl:attribute name="class">entryLinkA entryLinkA<xsl:value-of select="$referencing_column_name" /></xsl:attribute>
                                    <xsl:for-each select="//tables/table[@table_name=$referencing_table_name]/columns/column[@is_primary_key = 1 and @column_name != $referencing_column_name]">
                                        <xsl:element name="div">
                                            <xsl:attribute name="class">entryLinkIdentifierField<xsl:value-of select="$referencing_table_name" /><xsl:value-of select="@column_name" /></xsl:attribute>
