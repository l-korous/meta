...
<xsl:for-each select="_column[@_is_primary_key=1]" >
    , <xsl:value-of select="@_column" /> <xsl:value-of select="@_datatype" />
</xsl:for-each>
)
AS
BEGIN
    ...
    BEGIN TRANSACTION
       ...
       <xsl:for-each select="_column[@_is_primary_key=1]" >
            <xsl:value-of select="@_column" /> = @<xsl:value-of select="@_column" /> AND
        </xsl:for-each>
       branch_id = @branch_id) BEGIN
		  set @msg = 'ERROR: <xsl:value-of select="@_table" /> ( + 
          <xsl:for-each select="_column[@_is_primary_key=1]" >
            '<xsl:value-of select="@_column" />: ' + CAST(@<xsl:value-of select="@_column" /> AS NVARCHAR(MAX)) + ', ' +
        </xsl:for-each>
...
BEGIN
    ...
    SELECT
        <xsl:for-each select="_column" >
            _d.<xsl:value-of select="@_column" />,
        </xsl:for-each>
    ...
    INNER JOIN DELETED _d
	   ON
        ...
        AND _h.branch_id = 'master' and _h.valid_to is null
